# coding: utf-8

"""
    Invoices

    Use the Invoicing API to create, send, and manage invoices. You can also use the API or webhooks to track invoice payments. When you send an invoice to a customer, the invoice moves from draft to payable state. PayPal then emails the customer a link to the invoice on the PayPal website. Customers with a PayPal account can log in and pay the invoice with PayPal. Alternatively, customers can pay as a guest with a debit card or credit card. For more information, see the <a href=\"/docs/invoicing/\">Invoicing Overview</a> and the <a href=\"/docs/invoicing/basic-integration/\">Invoicing Integration Guide</a>.

    The version of the OpenAPI document: 2.3
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from pay_pal_invoicing_python_sdk.type.currency_code import CurrencyCode
from pay_pal_invoicing_python_sdk.type.file_reference import FileReference

class RequiredDetail(TypedDict):
    currency_code: CurrencyCode

class OptionalDetail(TypedDict, total=False):
    # The reference data. Includes a post office (PO) number.
    reference: str

    # A note to the invoice recipient. Also appears on the invoice notification email.
    note: str

    # The general terms of the invoice. Can include return or cancellation policy and other terms and conditions.
    terms_and_conditions: str

    # A private bookkeeping memo for the user.
    memo: str

    # An array of PayPal IDs for the files that are attached to an invoice.
    attachments: typing.List[FileReference]

class Detail(RequiredDetail, OptionalDetail):
    pass
